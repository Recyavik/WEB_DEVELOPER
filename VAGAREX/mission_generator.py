...
def _format_description(level: int, waypoints: list[list[float]],
    ...
    if level == 1:
        ...
        # L4 (углублённый): базовые за точки, бонус за точность,
        # штраф/доп. условия — обход опасных зон.
        parts.append(
            "⭐ За прохождение контрольных точек вы получите звёзды, "
            "не задевая опасные зоны. Точное следование эталонной траектории "
            "даёт дополнительные звёзды.")
    elif level == 5:
        # L5 (продвинутый): точки + установка attention + обход опасных;
        # точность даёт бонус.
        parts.append(
            "⭐ За прохождение контрольных точек, установку зоны внимания на "
            "финальной точке и обход опасных зон вы получите звёзды. "
            "Точное следование эталонной траектории даёт дополнительные звёзды.")
    else:
