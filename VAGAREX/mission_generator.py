...
def _format_description(level: int, waypoints: list[list[float]],
    ...
    if level == 1:
        ...
        # L3 (базовый): точки + точность следования по эталонной траектории.
        parts.append(
            "⭐ За правильное прохождение контрольных точек маршрута без "
            "отклонения от траектории вы получите звёзды.")
    elif level == 4:
        # L4 (углублённый): точки + точность траектории + не задевать опасные.
        parts.append(
            "⭐ За правильное прохождение контрольных точек маршрута без "
            "отклонения от траектории и без наезда на опасные зоны "
            "вы получите звёзды.")
    elif level == 5:
        # L5 (продвинутый): точки + точность + опасные + установка attention.
        parts.append(
            "⭐ За правильное прохождение контрольных точек маршрута без "
            "отклонения от траектории, установку зоны внимания на финальной "
            "точке и обход опасных зон вы получите звёзды.")
    else:
