...
def _candidate_set_course(state: dict, g: WorldGeom, rng: random.Random, *,
    ...
    Предпочитаем ГЛУБОКИЕ дуги — изменение курса на 90° (большой прогиб
    от хорды: срезать кривую по прямой невыгодно, теряется аккуратность).
    45° — запасной вариант, если дуга на 90° не помещается на поле.
    Резкие развороты (>90°) исключены — для них есть face/turn_around."""
    cur = state["heading"]
    deep: list[int] = []      # дуги на 90° — приоритет
    shallow: list[int] = []   # дуги на 45° — fallback
    for delta_abs in (90, 45):
        bucket = deep if delta_abs == 90 else shallow
        for sign in (+1, -1):
...
def _generate_level_2(geom: WorldGeom, rng: random.Random) -> dict:
    ...
    наезд на 2 остальные опасные зоны — −20% аккуратности.
    ...
    # обычные препятствия: наезд на них штрафует −20% аккуратности.
    if danger_zones:
...
def _generate_level_4(geom: WorldGeom, rng: random.Random) -> dict:
    """5-6 waypoints + 4 опасные зоны разного радиуса. Курсы кратны 15°
    (24 направления), точки не привязаны к сетке.
    ...
    Зоны не создаются и не удаляются игроком (actions_required пуст) —
    их нужно ОБХОДИТЬ, наезд штрафует аккуратность. Радиусы кратны 10:
    10/20/20/30 см. Старт генерируется случайно (_random_start_xy)."""
    n_waypoints = rng.randint(5, 6)
    s_x, s_y = _random_start_xy(rng)
    geom = dataclasses.replace(geom, start_x=s_x, start_y=s_y)
    traj = _generate_trajectory_best_of(
        n_waypoints, geom, rng,
    ...
    # 4 зоны разного радиуса. Каждый вызов получает уже размещённые
    # зоны в `existing` — чтобы зоны разных радиусов не пересекались.
    clearance = geom.safety_margin_cm + max(geom.robot_w_cm, geom.robot_l_cm) / 2.0
    zones: list[list[float]] = []
    for r in (10.0, 20.0, 20.0, 30.0):
        zones += _place_danger_zones(
            1, traj["full_path"], geom, rng,
            zone_radius_cm=r, clearance_cm=clearance, existing=zones)
...
def _generate_level_5(geom: WorldGeom, rng: random.Random) -> dict:
    """5 контрольных точек, эталонная траектория с длинными кривыми.
    5 опасных зон вдоль траектории: 2 удаляемые (remove_danger) +
    3 обходимые. 1 зона внимания (place_attention) — на ФИНИШНОЙ точке.

    Замысел: игрок пробирается по криволинейной траектории сквозь поле
    опасных зон, чтобы установить зону внимания на финише. Срезать кривые
    по прямой невыгодно — выход из коридора снижает аккуратность. Опасные
    зоны снимают ПОСЛЕ финиша (раннее снятие штрафует аккуратность).
    ...
    # 5 опасных зон вдоль траектории, радиусы кратны 10: 10/20/20/30/30.
    # Каждый вызов получает уже размещённые зоны в `existing` — зоны
    # не пересекаются друг с другом. Все вне коридора эталона: робот,
    # идущий точно по траектории, «пробирается» между ними.
    clearance = geom.safety_margin_cm + max(geom.robot_w_cm, geom.robot_l_cm) / 2.0
    zones: list[list[float]] = []
    for r in (10.0, 20.0, 20.0, 30.0, 30.0):
        zones += _place_danger_zones(
            1, traj["full_path"], geom, rng,
            zone_radius_cm=r, clearance_cm=clearance,
    ...
    actions: list[dict] = []
    # Зона внимания — одна, на ФИНИШНОЙ контрольной точке. Игрок
    # пробирается сквозь опасные зоны, чтобы установить её.
    if traj["waypoints"]:
        ax, ay = traj["waypoints"][-1]
        actions.append({"type": "place_attention",
                         "x": float(ax), "y": float(ay), "radius": 20.0})
    # 2 случайные опасные зоны — удаляемые (remove_danger). Остальные 3 —
    # обходимые препятствия (наезд штрафует, снять нельзя).
    if zones:
        for z in rng.sample(zones, min(2, len(zones))):
            actions.append({"type": "remove_danger",
                             "x": z[0], "y": z[1]})
...
def _format_description(level: int, waypoints: list[list[float]],
    ...
    if level == 1:
        ...
        parts.append(
            ...
            "зону, которую удалять не нужно, снижает аккуратность на 20%. "
        ...
        parts.append(
            "⭐ За прохождение контрольных точек вы получите звёзды. "
            "Точное следование эталонной траектории даёт дополнительные звёзды.")
    elif level == 4:
        # L4 (углублённый): базовые за точки, бонус за точность траектории,
        # штраф за наезд на опасные зоны (их только обходят).
        parts.append(
            "⭐ Звёзды — за контрольные точки. Опасные зоны нужно обходить: "
            "наезд на любую снижает аккуратность на 20%. Точное следование "
            "эталонной траектории даёт дополнительные звёзды.")
    elif level == 5:
        # L5 (продвинутый): траектория с кривыми + установка attention на
        # финише + снятие 2 опасных зон + обход остальных.
        parts.append(
            "⭐ Звёзды — за контрольные точки, установку зоны внимания на "
            "финише и снятие нужных опасных зон. Наезд на опасную зону, "
            "которую снимать не нужно, снижает аккуратность на 20%. "
            "ВАЖНО: снимать опасные зоны без штрафа можно ТОЛЬКО собрав все "
            "контрольные точки. Снятие раньше штрафует аккуратность на 20% за "
            "каждую зону. После сбора всех точек точность траектории больше "
            "не проверяется. "
            "Точное следование эталонной траектории даёт дополнительные звёзды.")
    else:
        # Кастомные миссии (level=0) и прочие
        parts.append(
    ...
    parts.append("💡 Каждая использованная подсказка снижает аккуратность на 20%.")
    return "\n".join(parts)
