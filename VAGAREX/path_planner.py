...
Алгоритм:
  ...
  2) Каждая клетка помечается как непроходимая, если в неё попадает
...
def _smooth_los(cells: List[Tuple[int, int]],
    ...
    i = 0
    while i < len(cells) - 1:
        # Ищем самую дальнюю клетку, до которой ещё «видно» по прямой.
        j = len(cells) - 1
        while j > i + 1:
...
def chaikin_smooth(points: List[Tuple[float, float]],
    ...
    """Алгоритм Чайкина: отрезает углы ломаной, превращая её в дугообразную
    кривую. Каждая итерация удваивает число точек и сглаживает повороты —
    робот, следующий за такой траекторией, будет крутить рулём непрерывно
...
def resample_curve(points: List[Tuple[float, float]],
    ...
    for i in range(len(points) - 1):
        ...
        if seg < 1e-6:
            continue
        # Сколько целых шагов укладывается с учётом «остатка» с прошлого сегмента
