...
""" % (c.move_speed, c.turn_angle, c.speed_at_100,
    ...
    def _battery_factor(charge: float) -> float:
        ...
        Имитирует поведение LiPo: до ~50% — практически номинал, дальше
        плавный спад, при 0% мотор не вращается.
        ...
        Кусочно-линейная модель:
          ...
          0% < x < 20%: 0.30 → 0.70
          0%: 0 (мёртвая батарея)"""
        if charge >= 50.0: return 1.0
        if charge >= 20.0: return 0.70 + 0.30 * (charge - 20.0) / 30.0
        if charge >  0.0:  return 0.30 + 0.40 * charge / 20.0
        return 0.0
